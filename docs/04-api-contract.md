...
| `X-Device-Id` | Да (везде, кроме `/health`) | **Произвольная непустая строка-идентификатор** устройства (напр. `testuser` или UUID). Постоянен; на iOS генерируется один раз и хранится в Keychain. Правила — ниже. |
...
- **Опаковый строковый идентификатор** (ADR-007): любая непустая строка в пределах правил ниже. UUID — валидный частный случай, но **не обязателен**: `testuser` так же допустим.
- **Хранится и отдаётся как есть** (echo): что прислали — то и вернётся в `device_id` (`GET /me`). Скрытого маппинга в UUID нет. Сравнение **регистрозависимое** (`TestUser` ≠ `testuser`).
- **Правила валидации** (сервер обрезает окружающие пробелы, затем проверяет результат):
  - непустой после trim → иначе `400 device_id_required`;
  - длина ≤ **64** символа → иначе `400 device_id_invalid`;
  - допустимые символы: ASCII буквы/цифры и `-` `_` `.` (`^[A-Za-z0-9._-]+$`); пробелы внутри, управляющие и иные символы (включая не-ASCII) → `400 device_id_invalid`.
...
| `400` | Некорректный запрос: отсутствует/пустой `X-Device-Id` (`device_id_required`); `X-Device-Id` нарушает правила — слишком длинный (>64) или недопустимые символы (`device_id_invalid`, **уже не означает «не UUID»**); нечитаемый JSON; отсутствует обязательное поле | `device_id_required`, `device_id_invalid`, `bad_request` |
...
```
- `device_id` — **echo присланного `X-Device-Id`** ровно как есть (опаковая строка, ADR-007). В примере выше — UUID, но это может быть любой валидный id, напр. `"testuser"`.
...
- **Device-id**: любая непустая строка ≤64 из `[A-Za-z0-9._-]` (напр. `testuser` для тестов или UUID для prod). Отдаётся обратно как есть. Для prod рекомендуется стабильный уникальный id (UUID), хранимый в Keychain; коллизия строк = общий доступ к данным. Не теряйте id при переустановке, иначе данные станут недоступны (Q-ID-1).
