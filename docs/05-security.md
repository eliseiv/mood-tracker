...
- Анонимная идентификация по заголовку `X-Device-Id` — **опаковая непустая строка** (не обязательно UUID), ADR-007. Проверяется **после** `X-API-Key`.
- Middleware: trim → валидация (непустой, ≤64, `^[A-Za-z0-9._-]+$`) → upsert `Device` → проброс `device_id` в request scope. Пусто → `400 device_id_required`; нарушение длины/charset → `400 device_id_invalid` (кроме `/health`).
- **Почему произвольная строка не вносит нового риска:** id опаков (не парсится, не используется в путях/командах), доступ скоупится по нему, app-level `X-API-Key` гейтит приложение. Ограничение длины (≤64) и charset (`[A-Za-z0-9._-]`, без управляющих/пробелов/не-ASCII) защищают от инъекций, log-forging и раздувания индекса; echo значения как есть безопасен (тот же ограниченный charset).
...
| Перебор/спуфинг device-id (опаковая строка) | Скоуп + `404` на чужое; app-key гейтит приложение; rate limit по device-id **и** IP; charset/длина ограничены (≤64, `[A-Za-z0-9._-]`) — нет инъекций/log-forging |
